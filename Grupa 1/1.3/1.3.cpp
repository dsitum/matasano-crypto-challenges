...
string DecryptOneCharKey(Message* message)
{
    float* score = new float[256];
    for (int i = 0; i < 256; i++)
    {
        string possibleOriginalMessage = Decrypt(message, (unsigned char) i);
    ...
    // all we have to do is decrypt character with this key
    string originalMessage = Decrypt(message, key);
...
int main()
{
    string msg = "1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736";
    Message* message = new Message(msg, Message::BASE16);
    string originalMessage = DecryptOneCharKey(message);
